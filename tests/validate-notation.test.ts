...
const GROUP_D = ['driver', 'actor', 'change', 'stakeholder', 'target-state', 'location', 'business-service', 'node'];
...
describe('validate-notation — element notation corpus validates clean (#518 C1)', () => {
  for (const notation of ['requirement', 'assertion', 'verification', 'risk', 'metric', 'need', 'validation', 'driver', 'actor', 'change', 'stakeholder', 'target-state', 'location', 'business-service', 'node'] as const) {
...
describe('validate-notation — parity with the preview validator (#258, #518 C1)', () => {
  ...
  it('node: CLI findings mirror validateNode exactly', () => {
