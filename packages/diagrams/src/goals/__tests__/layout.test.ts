...
describe('layoutGoalTree', () => {
  ...
  describe('layoutMode', () => {
    const branching: GoalTree = {
      ...
      goals: [
        { id: 1, name: 'Root', type: 'Strategy', level: 0, parent_id: 0 },
        { id: 5, name: 'Goal A', type: 'Business Goal', level: 1, parent_id: 1 },
        { id: 2, name: 'Goal B', type: 'Business Goal', level: 1, parent_id: 1 },
        { id: 3, name: 'Goal C', type: 'Business Goal', level: 1, parent_id: 1 },
        { id: 6, name: 'Project 1', type: 'Project', level: 2, parent_id: 5 },
        { id: 4, name: 'Project 2', type: 'Project', level: 2, parent_id: 2 },
      ],
    };

    it("mode 'standard' produces a valid layout", () => {
      const layout = layoutGoalTree(branching, { layoutMode: 'standard' });
      expect(layout.nodes).toHaveLength(6);
      expect(layout.edges).toHaveLength(5);
    });

    it("mode 'minimizeCrossings' produces the same node count", () => {
      const standard = layoutGoalTree(branching, { layoutMode: 'standard' });
      const optimized = layoutGoalTree(branching, { layoutMode: 'minimizeCrossings' });
      expect(optimized.nodes).toHaveLength(standard.nodes.length);
      expect(optimized.edges).toHaveLength(standard.edges.length);
    });

    it("mode 'minimizeCrossings' sorts siblings deterministically", () => {
      const l1 = layoutGoalTree(branching, { layoutMode: 'minimizeCrossings' });
      const l2 = layoutGoalTree(branching, { layoutMode: 'minimizeCrossings' });
      // Same input should produce identical layouts
      expect(l1.nodes.map(n => [n.id, n.x, n.y])).toEqual(
        l2.nodes.map(n => [n.id, n.x, n.y])
      );
    });

    it("mode 'minimizeCrossings' respects nodeWidth and nodeSep", () => {
      const compact = layoutGoalTree(branching, {
        layoutMode: 'minimizeCrossings',
        nodeWidth: 200,
        nodeSep: 50,
      });
      const standard = layoutGoalTree(branching, {
        layoutMode: 'minimizeCrossings',
        nodeWidth: 250,
        nodeSep: 24,
      });
      // Width difference should be visible (different nodeWidth + rankSep contributions)
      expect(compact.bounds.width).not.toBe(standard.bounds.width);
    });
  });
});
