...
| Adding a new notation | Pattern in [TX-020 archive](../.archive/tasks/TX-020.md): library module under `packages/diagrams/src/<notation>/`, preview under `extension/src/<notation>-preview.ts`, wiring in `extension/src/extension.ts` and `extension/package.json`, example in `examples/<notation>/`. |
